...
public class Regex {
    ...
    public static final Pattern PLATE_PATTERN = Pattern.compile("^[A-Z]{3}\\d{3}$|^[A-Z]{3}\\d{2}[A-Z]$|^[R|S]\\d{5}$");
    ...
    public static final Pattern DATE_PATTERN = Pattern.compile("^([0-2][0-9]|(3)[0-1])-(0[1-9]|1[0-2])-(\\d{4})$");
    // Hora en formato hh-mm-ss
    public static final Pattern TIME_PATTERN = Pattern.compile("^([01]?[0-9]|2[0-3]):([0-5]?[0-9]):([0-5]?[0-9])$");
    ...
    // URL
    public static final Pattern URL_PATTERN = Pattern.compile("^(https?|ftp)://[\\w\\.-]+(:\\d+)?(/[\\w\\.-]*)*/?$"); //^(https?|ftp):\/\/(([a-zA-Z0-9-]+\.)+[a-zA-Z]{2,})(:\d+)?(\/[^\s]*)?(\?[^\s]*)?(#[^\s]*)?$
    ...
    public static final Pattern BARCODE_PATTERN = Pattern.compile("^\\d{13}$"); //revisar
    ...
    public static final Pattern FOR_LOOP_PATTERN = Pattern.compile("^for\\s*\\(\\s*\\w+\\s*=\\s*\\d+\\s*;\\s*\\w+\\s*<\\s*\\w+\\s*;\\s*\\w+\\+\\+\\s*\\)$"); //^([01]?[0-9]|2[0-3]):([0-5]?[0-9]):([0-5]?[0-9])$
